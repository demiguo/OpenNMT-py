...
class GlobalAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None, q_scores_residual=None):
        ...
        raw_scores[2].data.masked_fill_(1 - mask, math.exp(-10))
        ...
        concat_c_input_feed = torch.cat([c_input_feed, input], 2).view(batch*targetL, dim*2)
        attn_h = self.linear_out(concat_c).view(batch, targetL, dim)
        attn_h_input_feed = self.linear_out(concat_c_input_feed).view(batch, targetL, dim)
        if self.attn_type in ["general", "dot"]:
            attn_h = self.tanh(attn_h)
            attn_h_input_feed = self.tanh(attn_h_input_feed)

        if one_step:
            attn_h = attn_h.squeeze(1)
            attn_h_input_feed = attn_h_input_feed.squeeze(1)
